...
bk=open(FileList)
iter=0
title_line='k-tuple'+'\t'+'number_of_occurrences'+'\t'+'M'+str(r)+'_possibility'
for fileName in bk.readlines():
	...
	'''
	tempFileName=TC_Path+sampleName+'_k'+str(r)+'_tupleCount.txt'
	temp_file=open(tempFileName, 'r')
	total=0
	r_count_dict[r]={}
	line=temp_file.readline()
	for lines in temp_file.readlines():
		...
		ktuple=fields[0]
		count=int(fields[1])
		r_count_dict[r][ktuple]=count
		total+=count
	r_total_dict[r]=total
	'''
	...
	#mp4=SeqSign.MarkovPossibility(r_total_dict[4], r_count_dict[4], r_count_dict[5], KTuple_count, 4)
	#mp5=SeqSign.MarkovPossibility(r_total_dict[5], r_count_dict[5], r_count_dict[6], KTuple_count, 5)
	#mp6=SeqSign.MarkovPossibility(r_total_dict[6], r_count_dict[6], r_count_dict[7], KTuple_count, 6)
	#mp7=SeqSign.MarkovPossibility(r_total_dict[7], r_count_dict[7], r_count_dict[8], KTuple_count, 7)
	#mp8=SeqSign.MarkovPossibility(r_total_dict[8], r_count_dict[8], r_count_dict[9], KTuple_count, 8)
	#r=k-2
	mp=SeqSign.MarkovPossibility(r_total_dict[r], r_count_dict[r], r_count_dict[r+1], KTuple_count, r)
	outFileName=MP_Path+sampleName+'_k'+str(k)+'_hao_wdpw.txt'
	...
	for ktuple in KTuple_list:
		...
		#write_lines+="\t%s" % (mp4.KTuple_possibility[ktuple])
		#write_lines+="\t%s" % (mp5.KTuple_possibility[ktuple])
		#write_lines+="\t%s" % (mp6.KTuple_possibility[ktuple])
		#write_lines+="\t%s" % (mp7.KTuple_possibility[ktuple])
		#write_lines+="\t%s" % (mp8.KTuple_possibility[ktuple])
		write_lines+="\t%s" % (mp.KTuple_possibility[ktuple])
